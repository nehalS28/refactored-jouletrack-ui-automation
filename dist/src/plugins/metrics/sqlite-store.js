...
export class SQLiteStore {
    ...
    `);
        const insertMany = this.db.transaction((records) => {
            for (const record of records) {
                stmt.run(record.testId, record.testName, record.status, record.durationMs, record.workerId ?? null, JSON.stringify(record.locatorMetrics), JSON.stringify(record.waitMetrics), new Date().toISOString());
    ...
    async saveSuiteMetric(metrics) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(metrics.suiteId, metrics.totalTests, metrics.passed, metrics.failed, metrics.skipped, metrics.totalDurationMs, metrics.parallelWorkers ?? null, new Date().toISOString());
    }
    /**
     ...
     */
    updateBaselineSync(testId, durationMs) {
        if (!this.db) {
            return;
        }
     ...
     * @param durationMs - Test duration in milliseconds
     */
    async updateBaseline(testId, durationMs) {
     ...
     */
    async getHistoricalAverage(testId) {
