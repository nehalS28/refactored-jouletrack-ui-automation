...
export class ActionHelperImpl {
    ...
    logger;
    retryConfig;
    constructor(driver, wait, logger, retryConfig) {
        this.driver = driver;
        this.wait = wait;
        this.logger = logger;
     ...
     */
    async executeWithRetry(action, locator, fn) {
     ...
     * @requirements 4.5
     */
    async executeWithRetryInternal(action, elementDescription, fn) {
        let lastError;
     ...
     * @requirements 4.4
     */
    async handleActionFailure(action, elementDescription, error) {
     ...
     * @requirements 4.3
     */
    logActionStart(action, elementDescription, additionalData) {
     ...
     * @requirements 4.3
     */
    logActionComplete(action, elementDescription, additionalData) {
 ...
 */
export function createActionHelper(driver, wait, logger, retryConfig) {
