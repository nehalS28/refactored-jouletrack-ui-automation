...
const VALID_STRATEGIES = ['css', 'data-testid', 'xpath', 'aria-label'];
/**
 * Validates a locator definition has all required fields.
 */
function validateLocator(locator, filePath, locatorPath) {
    const errors = [];
 ...
 */
function readYamlFiles(registryDir) {
    const files = [];
    const errors = [];
 ...
 */
function generateLocatorCode(locator, indent) {
    const lines = [
        `${indent}{`,
 ...
 */
function generateComponentCode(component, indent) {
 ...
 */
function generatePageCode(page, indent) {
 ...
 */
function generateTypeExports(files) {
    const lines = [
        '',
    ...
    for (const { domain, pages } of files) {
        ...
        for (const pageName of Object.keys(pages)) {
            ...
            lines.push(`export type ${domainPascal}${pagePascal}Elements = keyof typeof locators.${domain}.${pageName};`);
        }
    }
    return lines.join('\n');
}
...
export function generateLocators(options) {
    ...
    // Summary
    const totalLocators = files.reduce((sum, file) => sum + Object.values(file.pages).reduce((s, page) => s + Object.keys(page).length, 0), 0);
    console.log(`  ${files.length} domain(s), ${totalLocators} locator(s)`);
}
/**
 * Creates the generate:locators command for the CLI.
 */
export function createGenerateLocatorsCommand() {
    const command = new Command('generate:locators')
        .description('Generate typed TypeScript locators from YAML registry files')
        .option('-r, --registry <path>', 'Path to locator registry directory', 'src/locators/registry')
        .option('-o, --output <path>', 'Output file path', 'src/locators/generated/index.ts')
...
//# sourceMappingURL=generate-locators.js.map