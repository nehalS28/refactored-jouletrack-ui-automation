 ...
 */
async function buildReport(testMetrics, suiteMetrics, baselines, days) {
    const now = new Date();
 ...
 */
function buildPerformanceComparison(testMetrics, baselines) {
    ...
    for (const [testId, metrics] of testMap.entries()) {
        ...
        if (!baseline)
            continue;
        const avgCurrent = Math.round(metrics.reduce((sum, m) => sum + m.durationMs, 0) / metrics.length);
        const percentageDifference = Math.round(((avgCurrent - baseline.avgDurationMs) / baseline.avgDurationMs) * 100);
        let status;
 ...
 */
function identifySlowLocators(testMetrics) {
    const locatorStats = new Map();
    for (const metric of testMetrics) {
        if (!metric.locatorMetrics)
            continue;
 ...
 * Calculate wait inefficiencies
 */
function calculateWaitInefficiencies(testMetrics) {
    const waitStats = new Map();
    for (const metric of testMetrics) {
        if (!metric.waitMetrics)
            continue;
 ...
 * Calculate parallel execution efficiency
 */
function calculateParallelEfficiency(suiteMetric) {
 ...
 */
async function outputJsonReport(report, outputPath) {
 ...
 */
async function outputHtmlReport(report, outputPath) {
...
//# sourceMappingURL=metrics-report.js.map