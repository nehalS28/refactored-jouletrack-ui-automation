...
import { SQLiteStore, type LocatorMetricData, type WaitMetricData, type TestMetricRecord } from './sqlite-store.js';
 ...
 */
const DEFAULT_CONFIG: MetricsPluginConfig = {
  enabled: true,
...
class TestMetricsBuilder {
  ...
  private readonly locatorMetrics: LocatorMetricData[] = [];
  private readonly waitMetrics: WaitMetricData[] = [];
  ...
  addLocatorMetric(metric: LocatorMetricData): void {
    this.locatorMetrics.push(metric);
  }

  addWaitMetric(metric: WaitMetricData): void {
    this.waitMetrics.push(metric);
  }

  build(status: TestStatus, durationMs: number): TestMetricRecord {
    return {
      testId: this.testId,
...
export class MetricsPlugin implements Plugin {
  ...
  private readonly config: MetricsPluginConfig;
  private readonly logger: StructuredLogger;
  private currentTest: TestMetricsBuilder | null = null;
   ...
   */
  private async checkPerformanceRegression(testId: string, duration: number): Promise<void> {
   ...
   */
  async getBaseline(testId: string): Promise<number | null> {
