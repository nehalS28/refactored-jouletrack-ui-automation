 ...
 * @requirements 20.2
 */
export async function buildDependencyGraph(srcPath: string = process.cwd()): Promise<void> {
  ...
  try {
    ...
    for (const node of graph.nodes.values()) {
      if (node.type === 'feature') summary.features++;
      else if (node.type === 'step') summary.steps++;
      else if (node.type === 'page') summary.pages++;
      else if (node.type === 'locator') summary.locators++;
    }
